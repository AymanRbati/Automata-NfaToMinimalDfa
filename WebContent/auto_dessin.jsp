...
/*arr[0]=["q1","q2","",""];
 ...
 state=["q0","q1","q2","q3"];*/
 var arr=[[1,2],[1,3]];
 //console.log("looh"+arr.length);
qf="3"
 console.log("arrr"+(arr[0][0]+arr[0][1]));
 state=["1","1,2","3"];
 qf="[3,4]";
 qf=qf.replace("[","");
  qf=qf.replace("]","");
		...
		if(!drawn.contains(i)){
			for(l=0;l<state.length;l++){
				...
				paper.path("M" +(x[pos1]+p_x) + " " +(y[pos2]+p_y) + " L" + p_x+ " " +p_y).attr({stroke:'yellow', 'stroke-width': 2, 'arrow-end':
				    'open-wide-long'});
				if(state[arr[i][l]].length>1){
					n_arr=state[arr[i][l]].split(",");
					for(b=0;b<n_arr.length;b++){
						e_val="q"+n_arr[i]+" " ;
					}
					var t1=paper.text((x[pos1]+p_x),(y[pos2]+p_y),"{"+e_val+"}").attr({"font-size":20});
					}//lengthb state
				var t1=paper.text((x[pos1]+p_x),(y[pos2]+p_y),"q"+arr[i][l]).attr({"font-size":20});
				...
				paper.path("M" +(x[pos1]+p_x) + " " +(y[pos2]+p_y) + " L" + p_x+ " " +p_y).attr({stroke:'yellow', 'stroke-width': 2, 'arrow-end':
				    'open-wide-long'});
				if(state[arr[i][j]].length>1){
					n_arr=state[arr[i][j]].split(",");
					for(b=0;b<n_arr.length;b++){
						e_val="q"+n_arr[i]+" " ;
					}
					var t1=paper.text((x[pos1]+p_x),(y[pos2]+p_y),"{"+e_val+"}").attr({"font-size":20});
			...
			f=exist();
				...
				paper.path("M" +(p_x) + " " +(p_y) + " L" + (p_x+x[pos1])+ " " +(p_y+y[pos2])).attr({stroke:'yellow', 'stroke-width': 2, 'arrow-end':
				    'open-wide-long'});
				var t1=paper.text((x[pos1]+p_x),(y[pos2]+p_y) ,"q"+arr[i][j]).attr({"font-size":20});
				var mySet = paper.set();
				mySet.push(c1,t1);
